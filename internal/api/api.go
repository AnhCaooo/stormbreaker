...
func GetTodayTomorrowPrice(w http.ResponseWriter, r *http.Request) {
	var reqBody, err = electric.BuildTodayTomorrowAsBodyRequest()
	if err != nil {
	...
	if err := json.NewEncoder(w).Encode(todayTomorrowPrice); err != nil {
		logger.Logger.Error("failed to encode response data", zap.Error(err))
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	// todo: maybe good to have a handling to split the response from list of array into 2 struct Today and Tomorrow. By doing this, client can easily visualize the view
